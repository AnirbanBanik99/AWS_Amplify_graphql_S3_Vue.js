<template>
  <div id="app">
    <div class="app-header">
      ...
      <v-card>
        <center><h1>Welcome to Memories</h1></center>
      </v-card>
    </div>
    <amplify-authenticator>
      <div>
        ...
        <form v-on:submit.prevent autocomplete="off">
          ...
          <button @click='createMemories' class='button'>Create</button>
      ...
      <div class="app-body">
        ...
        <div class="card-container" v-if="!loading">
          <div class="card" v-for="memory of sortedMemories" :key="memory.id">
            <div class="remove"><button @click='deleteMemories(memory)' class='button'>Delete</button></div>
            <div class="name">{{ memory.city }}</div>
            <div class="price">{{ memory.name }}</div>
            <div class="symbol">{{ memory.description }}</div>
          </div>
        </div>
      </div>
    </amplify-authenticator>
...
import { listMemories } from './graphql/queries';
import { createMemories, deleteMemories } from './graphql/mutations';
import { onCreateMemories,  onDeleteMemories } from './graphql/subscriptions';
...
export default {
  ...
  created() {
    ...
    API.graphql(graphqlOperation(onCreateMemories))
    .subscribe((sourceData) => {
      const newMemory = sourceData.value.data.oncreateMemories
      if (newMemory) {
        // skip our own mutations and duplicates
        if (this.memories.some(r => r.id == newMemory.id)) return;
        this.memories = [newMemory, ...this.memories];
      } 
    });

    API.graphql(graphqlOperation(onDeleteMemories))
    .subscribe((sourceData) => {
      const deletedMemory = sourceData.value.data.onDeleteMemories
      if (deletedMemory) {
        this.memories = this.memories.filter((r) => r.id != deletedMemory.id);
      } 
    });
  },
    methods: {
    async getData() {
      try {
        this.loading = true;
        const response = await API.graphql(graphqlOperation(listMemories));
        this.memories = response.data.listMemories.items;
      }
      catch (error) {
    ...
    async createMemories() {
      ...
      const memory = { name, description, city, clientId: this.clientId };
      try {
        const response = await API.graphql(graphqlOperation(createMemories, { input: memory }))
        console.log('Item created!')
        this.memories = [...this.memories, response.data.createMemories];
        this.form = { name: '', description: '', city: '' };
      } catch (error) {
        console.log('Error creating Memory...', error)
      }
    },
    async deleteMemories(memory) {
      if (memory) {
        try {
          const { id } = memory;
          await API.graphql(graphqlOperation(deleteMemories, { input: { id: id } }))
          console.log('Item deleted!')
          this.memories = this.memories.filter((r) => r.id != memory.id );
        } catch (error) {
